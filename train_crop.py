...
from dataloader_crop import datset
import cv2
...
parser.add_argument('--save_folder_u', default='./checkpoint_crop/', help='Location to save checkpoint models')
parser.add_argument('--save_folder_v', default='./checkpoint_crop/', help='Location to save checkpoint models')
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        input = torch.cat([nir, uv,occ], 1)
        input = input*masks
        # print(input.shape,masks.shape, (input*masks).shape)
...
def test(testing_data_loader,epoch):
    psnr_test= 0
    model.eval()
    idx = 0
    for batch in testing_data_loader:
        ...
        with torch.no_grad():
            ...
            prediction = prediction 
            gt = gt[:,1:] 
        if idx<20 & epoch%10==0:
            out = torch.cat([nir,prediction],1)
            gt_out = torch.cat([nir,gt],1)
            # cv2.imwrite('/home/dx/usrs/dongkun/xk/colour/result/train_out/'+str(epoch).zfill(3)+'_'+str(idx).zfill(5)+'.png',(torch.cat([out[0], gt_out[0]],1).permute(1, 2, 0)[:,:,1].cpu().numpy()*255).astype('uint8'))
            cv2.imwrite('/home/dx/usrs/dongkun/xk/colour/result/train_out/'+str(epoch).zfill(3)+'_'+str(idx).zfill(5)+'.png',cv2.cvtColor((torch.cat([(gt_out*masks)[0], (out*masks)[0]],1).permute(1, 2, 0).cpu().numpy()*255).astype('uint8'),cv2.COLOR_YCrCb2RGB))
        idx+=1
        # loss = criterion(prediction, gt)
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.start_iter, opt.nEpochs + 1):
        train(epoch)
        psnr = test(testing_data_loader,epoch)
        scheduler.step()
        PSNR.append(psnr)
        ...
        if min_psnr==-1:
            ...
            torch.save(model.state_dict(), os.path.join(opt.save_folder_u, lastest_model))
