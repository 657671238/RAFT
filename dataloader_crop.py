...
class datset(Dataset):
    ...
    def __init__(self, train=True, size=14):
        ...
        self.list1 = np.asarray(self.list1).repeat(5)
        self.list2 = np.asarray(self.list2).repeat(5)
        self.list3 = np.asarray(self.list3).repeat(5)
        ...
        self.list4 = np.asarray(self.list4).repeat(5)
        ...
        self.list5 = np.asarray(self.list5).repeat(5)

        self.mask = '/home/dx/usrs/dongkun/xk/colour/result/crop_mask'
        list6 = os.listdir(self.mask)
        list6.sort(key=lambda x: int(x[:-4]))
        self.list6 = [os.path.join(self.mask, img) for img in list6]

        self.start = 0
        self.lens = 900*5
        if train == False:
            self.start = self.lens
            self.lens = len(self.list1)-self.lens
        random.seed(2)
    ...
    def __getitem__(self, idx):
        ...
        masks = self.list6[self.start+idx]

        # print(nir, uv, occ, gt, right, masks)
        ...
        return {'gt': gt, 'nir': nir, 'uv': uv, 'occ': occ,'right': right, 'masks': masks}
