...
def load_image(imfile):
    ...
    # cv2.imwrite('./right_yuv_pre.png',img)
    # img = cv2.cvtColor(img, cv2.COLOR_YCR_CB2RGB)
    # cv2.imwrite('./right_yuv_rgb.png',img)
    # exit()
...
if __name__ == '__main__':
    ...
    for i in range(len(img1s)):
        ...
        if i==1047:
            # img1 = root1+'/'+img1s[i]
            # img2 = root2+'/'+img2s[i]
            img1 = '/home/dx/usrs/dongkun/dataset/GT/level0/sintel/color_nir/01043.png'
            img2 = '/home/dx/usrs/dongkun/dataset/noise/sintel/color_1/01043.png'
            time1 = time.clock()
            # index = '%04d' % (i+1)
            # print(index)
            ...
            # print(ori.shape, flow_l2r.shape, flow_up_12r2l.shape)
            cv2.imwrite('./r2l.png',cv2.cvtColor(ori[0],cv2.COLOR_YCrCb2RGB))

            ori, flow_l2r, flow_up_12r2l = feature_warp(iml,imr,iml)
            # print(ori.shape, flow_l2r.shape, flow_up_12r2l.shape)
            # cv2.imwrite(result_save_path+'/l2r.png',cv2.cvtColor(ori[0],cv2.COLOR_BGR2RGB))
            # cv2.imwrite(result_save_path+'/l2r0.png',ori[0][:,:,0])
            # cv2.imwrite(result_save_path+'/l2r1.png',ori[0][:,:,1])
            ...
            print(occultation_l2r.shape)
            cv2.imwrite('./occ.png',occultation_l2r, [cv2.IMWRITE_PNG_COMPRESSION, 0])
            exit()
            time2 = time.clock()
            ...
            cv2.imwrite('./last.png',image, [cv2.IMWRITE_PNG_COMPRESSION, 0])
            exit()
